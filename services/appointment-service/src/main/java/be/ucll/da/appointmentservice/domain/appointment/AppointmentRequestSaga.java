...
public class AppointmentRequestSaga {
    ...
    public void executeSaga(Integer id, DoctorsOnPayrollEvent event) {
        ...
        if (selectedDoctor != null) {
            ...
            eventSender.sendEmail(appointment.getPatientEmail(), generateMessage(appointment.getId(), "You cannot book an appointment, we have no doctors for your requested expertise."));
        }
    }
    ...
    public void executeSaga(Integer id, RoomReservedEvent event) {
        ...
        if (event.getRoomAvailable()) {
            appointment.roomAvailable(event.getId());
    ...
    public void executeSaga(Integer id, PatientAccountNotCreatedEvent event) {
        Appointment appointment = getAppointmentById(id);
        appointment.noInsurance();
        eventSender.sendReleaseRoomCommand(appointment.getRoomId(), appointment.getPreferredDay());
    ...
    public void executeSaga(Integer id, PatientAccountCreatedEvent event) {
        ...
        // Final check to make sure that another request that went through in parallel also succeeds
    ...
    public void decline(Integer id) {
        ...
        appointment.decline();
        eventSender.sendReleaseRoomCommand(appointment.getRoomId(), appointment.getPreferredDay());
        eventSender.sendCloseAccountCommand(appointment.getId(), appointment.getPatientId());
    }
